...
# uses generic growth of stats based on level plus bonus growth based on rarity
#func _calc_stat(base: float, growth_rate : float) -> float:
	#return base + growth_rate * (level - 1) + rarity * STAT_RARITY_SCALE * level
